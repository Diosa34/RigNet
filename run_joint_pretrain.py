...
def train(train_loader, model, optimizer, args):
    ...
    if args.use_moe and lb_loss_meter.count > 0:
        ...
        print('  moe_load_balance_loss: {:.6f} (~1.0 per MoE layer when balanced)'.format(
            lb_loss_meter.avg))
    if args.use_moe and usage_spread_meter.count > 0:
        moe_metrics['expert_usage_spread'] = usage_spread_meter.avg
        print('  moe_expert_usage_spread: {:.4f} (target > 0.1 for specialization)'.format(
            usage_spread_meter.avg))
    if args.use_moe and router_entropy_meter.count > 0:
        moe_metrics['router_entropy'] = router_entropy_meter.avg
        print('  moe_router_entropy: {:.4f} (lower => sharper routing)'.format(
            router_entropy_meter.avg))
